...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef* huart, uint16_t Size)
{
	...
	{
		cnt++;
		std::memcpy(rx_data_, rx_buf_, RC_RX_BUF_SIZE);
		HAL_UARTEx_ReceiveToIdle_DMA(&huart1, rx_buf_, RC_RX_BUF_SIZE);
...
void RC::frameHandle()
{
	...
	channel_.l_col = linearMappingInt2Float((uint16_t)(rx_data_[1] & 0x1f) << 6 | rx_data_[2] >> 2, 364, 1684, -1.0,
											1.0);
	channel_.r_col = linearMappingInt2Float(
		(uint16_t)(rx_data_[2] & 0x03) << 9 | (uint16_t)rx_data_[3] << 1 | rx_data_[4] >> 7, 364, 1684, -1.0, 1.0);
	channel_.l_row = linearMappingInt2Float((uint16_t)(rx_data_[4] & 0x7f) << 4 | rx_data_[5] >> 4, 364, 1684, -1.0,
											1.0);
